...
/* IMPORTANT: 
Unable to use vscode scripts on this file.
To use do: 
    vscode.postMessage({
        type: "xxxxxxx",
    });
Then go to chatgpt-view-provider.ts and add case "xxxxxxx": 
 */
...
(function () {
    ...
    // START OF: Handle messages sent from the extension to the webview ------------------------------------------------------------------------------------------
        ...
        switch (message.type) {
            case "showInProgress":
                ...
                if (message.inProgress) {
                    ...
                    document
                        .getElementById("question-input-buttons")
            ...
            case "addQuestion":
                ...
                list.innerHTML += `<div class="p-4 self-end mt-4 question-element-ext relative input-background">
                        ...
                        <div class="overflow-y-auto">${escapeHtml(message.value)}</div></div>`;
            ...
            case "addError":
                ...
                list.innerHTML += `<div class="p-4 self-end mt-4 pb-8 error-element-ext" data-license="isc-gnc">
                        <h2 class="mb-5 flex">${aiSvg}ChatGPT</h2>
            ...
            case "loginSuccessful":
                ...
                document.getElementById("notebook-guidance-button")?.classList.remove("hidden");
    ...
    // END OF: Handle messages sent from the extension to the webview ------------------------------------------------------------------------------------------

    const startNotebookGuidance = () => {
        let userInput = '';
        let num_prompts = 0;
        let current_step = 0;
        const escapeHtml = (unsafe) => {
        ...
        const more_detail_prompt = `
                \nAsk the user to provide you with more detail about each dataset e.g.,
                Can you please provide more detail about each dataset and their variables?
                Do not generate extra informations.
                `;
        const goal_prompt = `
                \nAsk the user what their goal is e.g.,
                What is your goal for this task based on the data provided?
                `;
        const general_step_prompt = `
                \nGenerate the required step (with number) to achieve the user's goal as detail as possible.
                Do not generate any code.
                `;
        const start_instruction = `
        \nDo you want to start the creating the notebook? If so type 'Yes'.
        `;
        custom_prompts.push(more_detail_prompt);
        custom_prompts.push(goal_prompt);
        custom_prompts.push(general_step_prompt);
        const list = document.getElementById("qa-list");
        list.classList.remove("hidden");
        document.getElementById("introduction")?.classList?.add("hidden");
        document.getElementById("conversation-list")?.classList?.add("hidden");
        document.getElementById("default-question-container").classList.add("hidden");
        document.getElementById("modified-question-container").classList.remove("hidden");
        const welcomeMsg = `
            Welcome! I am Notebook Whisperer. Your personal Jupyter Notebook assistant.<br>
            To get started. Please provide me the context of your notebook and the datasets.<br>
            Here is an example:<br>
            The notebook I am writing is about .... These are the given datasets:<br>
            games.csv - metadata about games<br>
            turns.csv - all turns from start to finish of each game<br>`;
        // Initiate the UI
        list.innerHTML += `<div data-license="isc-gnc" class="p-4 self-end mt-4 pb-8 answer-element-ext">
                       <h2 class="mb-5 flex">${aiSvg}ChatGPT</h2>
                         <div class="result-streaming" id="">${welcomeMsg}</div>
                    </div>`;
        document.getElementById("guidance-input").addEventListener("keydown", function (event) {
            // If the user presses Enter
            if (event.key === "Enter" && !event.shiftKey && !event.isComposing) {
                event.preventDefault();
                const input = document.getElementById("guidance-input");
                userInput = input.value; // Store the user input
                // Display the user input to the UI
                list.innerHTML += `<div class="p-4 self-end mt-4 question-element-ext relative input-background">
                        <h2 class="mb-5 flex" data-license="isc-gnc">${userSvg}You</h2>
                        <no-export class="mb-2 flex items-center" data-license="isc-gnc">
                            <button title="Edit and resend this prompt" class="resend-element-ext p-1.5 flex items-center rounded-lg absolute right-6 top-6">${pencilSvg}</button>
                        ...
                        <div class="overflow-y-auto">${escapeHtml(userInput)}</div></div>`;
                input.value = ''; // Clear the input field
                if (userInput == 'Yes' || userInput == 'Continue') {
                    current_step += 1;
                    vscode.postMessage({
                        type: "startGuiding",
                        value: "What do I need to do for step " + current_step.toString() + ". If possible, generate some code to demonstrate.",
                    });
                } else {
                    // Modified the user input
                    if (num_prompts < custom_prompts.length) {
                        vscode.postMessage({
                            type: "addGuidanceQuestion",
                            value: userInput + custom_prompts[num_prompts],
                        });
                    } else {
                ...
                // }
                num_prompts += 1;
                // Our own version to retrieve the response from the API so we can customize it
                window.addEventListener("message", (event) => {
                    const message = event.data;
                    const list = document.getElementById("qa-list");
                    switch (message.type) {
                        case "addResponsev2":
                            ...
                            if (message.done) {
                                const preCodeList = list.lastChild.querySelectorAll("pre > code");
                                // Modified gpt reponse
                                if (num_prompts == custom_prompts.length) {
                                    list.innerHTML += `<div data-license="isc-gnc" class="p-4 self-end mt-4 pb-8 answer-element-ext">
                                                <h2 class="mb-5 flex">${aiSvg}ChatGPT</h2>
                                                <div class="result-streaming" id="${message.id}">${markedResponse + start_instruction}</div>
                                            </div>`;
                                } else {
                                    list.innerHTML += `<div data-license="isc-gnc" class="p-4 self-end mt-4 pb-8 answer-element-ext">
                                                <h2 class="mb-5 flex">${aiSvg}ChatGPT</h2>
    ...
    const clearConversation = () => {
        ...
        document.getElementById("default-question-container").classList.remove("hidden");
        document.getElementById("modified-question-container").classList.add("hidden");
        vscode.postMessage({
    ...
    // START OF: addEventListener for HTML elements in chat-gpt-view-provider.ts ------------------------------------------------------------------------------------------
    document.addEventListener("click", (e) => {
        ...
        if (e.target?.id === "notebook-guidance-button") {
            e.preventDefault();
            startNotebookGuidance();
            return;
        }
    ...
    // END OF: addEventListener for HTML elements in chat-gpt-view-provider.ts ------------------------------------------------------------------------------------------
})();
